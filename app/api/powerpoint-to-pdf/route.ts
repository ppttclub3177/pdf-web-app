...
export async function POST(request: Request): Promise<Response> {
  const response = await withApiErrorHandling(async () => {
    return withTempDir("powerpoint-to-pdf", async (workDir) => {
      const formData = await request.formData();
      const file = getRequiredFile(formData, "file");
      assertAllowedExtensions(file, [".ppt", ".pptx"]);
      ...
      return pathToFileResponse(outputPath, "slides.pdf", "application/pdf");
    });
  });
